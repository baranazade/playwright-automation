...
test.describe("WebTables Tests ", () => {
  ...
  test("Read all the data from the web table", async ({ page }) => {
    let table = page.locator("//table[@id='ctl00_MainContent_orderGrid']");
    let rows = await table.locator("//tr").all(); // get all rows of data from the web table

    let rowsData = [[], [], [], [], [], [], []];

    for (let i = 0; i < rows.length; i++) {
            let columns = await rows[i].locator("//td").all(); // get all columns of each row
            for (let j = 1; j < columns.length-1; j++) {
                let cellText = await columns[j].textContent(); // get the text content of each cell
  ...
  test("Check each checkbxoes on the web table", async ({ page }) => {
    ...
    let table = page.locator("//table[@id='ctl00_MainContent_orderGrid']");
    let checBoxes = await table.locator("//tr//td/input[@type='checkbox']").all();
    for(let checBox of checBoxes){
